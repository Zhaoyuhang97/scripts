...
def home_page():
    with use_scope("main_content"):  # 在 "main_content" 作用域中渲染内容
        ...
        # 车型展示
        put_row([
            put_text("系列车型").style("text-align: center; margin: 4rem 0; font-size: 2rem;")
        ])
        models = [
        ...
        put_grid([
            [put_column([
                ...
                <div style="text-align: center; padding: 1rem;">
                    <h3>{model['name']}</h3>
                    <p style="color: #666; margin: 0.5rem 0;">{model['desc']}</p>
                    <p style="font-weight: bold;">{model['price']}</p>
                    <button onclick="showModelDetail('{model['name']}')" style="margin-top: 1rem; border: 1px solid #0071e3; color: #0071e3; padding: 0.5rem 1.5rem; border-radius: 20px; cursor: pointer;">了解更多</button>
                </div>
                """)
